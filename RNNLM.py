...
class RNNLM_Model(nn.Module):
  ...
  def __init__(self, config):
    ...
    self.embedding_layer = nn.Embedding(self.config.vocab_size, self.config.embed_size)

    ### Define the H, I, b1 in HW4. Hint: check nn.Parameter
    self.H=nn.Parameter(torch.ones(self.config.hidden_size,self.config.hidden_size))
    self.I=nn.Parameter(torch.ones(self.config.embed_size,self.config.hidden_size))
    self.b1=nn.Parameter(torch.ones(self.config.hidden_size,))

    ### Define the projection layer, U, b2 in HW4
    self.U=nn.Parameter(torch.ones(self.config.hidden_size,self.config.vocab_size))
    self.b2=nn.Parameter(torch.ones(self.config.vocab_size,))
    self.projection_layer=(torch.ones(self.config.batch_size, self.config.vocab_size))
  ...
  def add_embedding(self, input_x):
    ...
    for i in range(self.config.num_steps):
        inputs.append(torch.squeeze(self.embedding_layer(torch.split(input_x,1,1)[i])))
    input_x=inputs
    ### END YOUR CODE
    return input_x
  ...
  def add_model(self, input_x, initial_state):
    ...
    initial_state = nn.Parameter(torch.zeros(self.config.batch_size, self.config.hidden_size)).cuda()
    output=[]
    curr_hidden_state = initial_state
    m=nn.Sigmoid()
    for x in input_x:
        curr_hidden_state= m(torch.matmul(x,rnn_i1)+torch.matmul(curr_hidden_state, rnn_h1)+rnn_b1)
        output.append(curr_hidden_state)
    self.final_state = curr_hidden_state
    rnn_outputs=[self.output_drop(x) for x in output]
    ### END YOUR CODE
    return rnn_outputs, self.final_state
  ...
  def add_projection(self, rnn_outputs):
    ...
    for output in rnn_outputs:
        outputs.append(torch.matmul(output, self.U)+self.b2)
...
def compute_loss(outputs, y, criterion):
  ...
  """ 
  ### YOUR CODE HERE
  loss=criterion(torch.cat(outputs), torch.t(y).flatten())
...
def test_RNNLM():
  ...
  model_optimizer = torch.optim.SGD(our_model.parameters(), lr=0.01, momentum = 0, weight_decay=1e-4)
  ### END YOUR CODE
